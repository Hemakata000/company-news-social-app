    ...
    try {
      ...
      const completion = await this.client.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are an expert business analyst who extracts key highlights from company news articles. You provide structured, actionable insights that professionals can use for decision-making and social media sharing.'
          },
          {
            role: 'user',
            content: prompt
      ...
      if (!responseContent) {
        throw this.createAIServiceError('EMPTY_RESPONSE', 'OpenAI returned empty response');
      }

      const highlights = this.parseHighlightsResponse(responseContent);
      ...
      if (error instanceof Error && 'code' in error) {
        throw this.createAIServiceError(
          error.code as string || 'OPENAI_ERROR',
          `OpenAI API error: ${error.message}`,
          error
        );
      }
      throw this.createAIServiceError('UNKNOWN_ERROR', 'Unknown error during highlight extraction', error as Error);
    }
  }
    ...
    try {
      ...
      const completion = await this.client.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a social media content expert who creates engaging, platform-specific posts from business news highlights. You understand each platform\'s audience, character limits, and best practices for engagement.'
          },
          {
            role: 'user',
            content: prompt
      ...
      const posts = this.parseSocialContentResponse(responseContent, request.platforms);
      const processingTime = Date.now() - startTime;
      ...
      throw this.createAIServiceError('UNKNOWN_ERROR', 'Unknown error during social content generation', error as Error);
    }
  }
